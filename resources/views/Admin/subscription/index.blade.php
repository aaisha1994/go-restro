                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col">
                                    <div class="d-grid gap-2 d-flex justify-content-end">
                                        <a href="{{ route('admin.subscription.history') }}" class="btn btn-sm btn-primary me-md-2"  type="button">History</a>
                                        <a href="{{ route('admin.subscription.create') }}" class="btn btn-sm btn-primary" type="button">&#43; Add Subscription</a>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($Subscriptions as $Subscription)
                                            <tr id="row_{{ $Subscription->id }}">
                                                ...
                                                <td>{{ $Subscription->tenure }}</td>
                                                <td>{{ $Subscription->amount }}</td>
                                                {{-- <td>ENJOY10</td>
                                                <td>50 Days</td> --}}
                                                <td id="tooltip-container9">
                                                    <a href="{{ route('admin.subscription.edit',[$Subscription->id]) }}" class="me-1 btn btn-sm btn-secondary" data-bs-container="#tooltip-container9" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Edit" data-bs-original-title="Edit">
        ...
        $(document).on('click', '.delete', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "delete",
                        url: "{{ route('admin.subscription.delete', ['_id']) }}".replace('_id', id),
                        dataType: "json",
